...
async fn admin_status_page_shows_runtime_state() {
    ...
    assert!(!body.contains("Base URL"));
    assert!(!body.contains("Bind"));
    assert!(!body.contains("Admin enabled"));
    assert!(!body.contains("Xattr namespace"));
    assert!(!body.contains("Upload minimum free percent"));
    assert!(!body.contains("Max connections"));
    assert!(!body.contains("Change log retention days"));
    assert!(!body.contains("Change log min entries"));
...
async fn admin_settings_page_is_read_only_config_view() {
    ...
    assert!(body.contains("notify_push_advertised_types"));
    assert!(body.contains("admin_enabled"));
    assert!(body.contains("Xattr namespace"));
    assert!(body.contains("Database"));
...
#[tokio::test]
async fn search_returns_friendly_not_implemented() {
    let (app, _temp, password) = app_with_temp_root().await;
    let response = app
        .oneshot(
            Request::builder()
                ...
                .header(header::AUTHORIZATION, auth_header(&password))
                .header(header::CONTENT_TYPE, "application/xml")
                .body(Body::from("<d:searchrequest xmlns:d=\"DAV:\"/>"))
                .unwrap(),
        )
        .await
        .unwrap();
    ...
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let body = std::str::from_utf8(&body).unwrap();
    assert!(body.contains("<d:supported-method/>"));
    assert!(body.contains("WebDAV SEARCH is not enabled"));
    assert!(body.contains("oc:filter-files REPORT"));
}
